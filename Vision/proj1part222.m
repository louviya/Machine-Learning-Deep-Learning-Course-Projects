function [PP,errorpp]=proj1part222(addt)
...
%recompute
len=length(addt(:));
mv =mm([addt],:); %validation set mv for 2D
Mv =MM([addt],:); %validation set Mv for 3D
mt = mm;
mt([addt],:)=[]; %training set mt for 2D
Mt = MM;
Mt([addt],:)=[]; %training set Mt FOR 3D
...
PP= h2d \ [kesai*p1' kesai*p14; kesai*p2' kesai*p24;kesai*p3' kesai*p34;] * h3d;
