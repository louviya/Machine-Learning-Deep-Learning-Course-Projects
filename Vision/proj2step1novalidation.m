function [param]=proj2step1novalidation()
%K=48;
%N=6;
...
t=zeros(1,3);
for i=1:K
    t = [MM(i, 1) MM(i, 2) MM(i, 3)].*mm(i,1);
    t2= [MM(i, 1) MM(i, 2) MM(i, 3)].*mm(i,2);
    A(j, :)=[MM(i, 1) MM(i, 2) MM(i, 3) 1 0 0 0 0 -t -mm(i, 1)];
    A(j+1, :)=[0 0 0 0 MM(i, 1) MM(i, 2) MM(i, 3) 1 -t2 -mm(i, 2)];
    j=j+2;
    if (j==(2*K-1)) 
        break; 
    end
...
param =[r1 r2 r3; tx ty tz fu fv u0 v0 0 0];
%tz <0, all take negate
end
